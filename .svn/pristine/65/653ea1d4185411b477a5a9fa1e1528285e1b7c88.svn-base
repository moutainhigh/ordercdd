...
public class LoanoverdueScheduled {
	...
	private static final long INTERVAL = 1000*60*60*24*3L;//3天
	//还款计划表状态
	...
	@Resource
	private WlqzWechatPubService wechatService;
	...
	private void checkAndSaveIsOverdue(Creditrepayplan plan) {
		...
		//没有逾期
		long curStamp = currentDate.getTime();
		long planStamp = planDate.getTime();
		if ((planStamp - curStamp) < INTERVAL) {//3天之内提醒
			wechatService.pushOverdueMsg(loan.getDebtorUser(), null, loan.getPrincipal(), 
					plan.getFees(), plan.getLeftAmount(), DateFormatUtils.format(plan.getDebtorRepaymentDate(), "yyyy年MM月dd日 HH:mm"));
		}
	}
