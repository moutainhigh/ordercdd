...
public interface OrdercddRepository extends WorkflowEntityRepository<Ordercdd>,SpecificationQueryRepository<Ordercdd> {
...
//    Ordercdd findByApplyIdentityNo
    ...
    @Query("select o from Ordercdd o where o.serviceUser.id=:userid")
    public Page<Ordercdd> getOrdercddByServiceUser(@Param("userid") Integer userid, Pageable pageable);

    @RestResource(exported = false)
    List<cddCountProjection> findAllByServiceUserAndStateIn(User serviceUser,Collection<OrdercddState> states);

    //根据UserId查询订单数
    @Query(value = "select o from Ordercdd o where o.user.id=?1")
    public List<Ordercdd> getOrdercddByUserId(Integer userId);
    
    @Query("select o from Ordercdd o join o.createdByDepartment d where d.id=?1")
    public List<Ordercdd> findByCreatedByDepartmentId(Integer departmentId);
}
