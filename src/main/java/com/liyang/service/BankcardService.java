...
public class BankcardService extends AbstractAuditorService<Bankcard, BankcardState, BankcardAct, BankcardLog> {
    ...
    @Autowired
    BankcardStateRepository bankcardStateRepository;
    ...
    public Bankcard saveByOrdercdd(Ordercdd ordercdd) {

        Bankcard bankcard = bankcardRepository.findBankcardByAccountIdentity(ordercdd.getApplyBankCard());//网上说银行卡号是唯一的
        if (bankcard == null) {
            bankcard = new Bankcard();
        }
        bankcard.setUser(ordercdd.getUser());
        bankcard.setRealName(ordercdd.getRealName());
        bankcard.setAccountIdentity(ordercdd.getApplyBankCard());
        bankcard.setBranchName(ordercdd.getApplyBankCardBranch());

        bankcard.setStatus(1);
        bankcard.setState(bankcardStateRepository.findByStateCode("ENABLED"));
