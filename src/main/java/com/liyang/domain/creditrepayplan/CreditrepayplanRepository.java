...
public interface CreditrepayplanRepository extends WorkflowEntityRepository<Creditrepayplan>,SpecificationQueryRepository<Creditrepayplan> {
	...
	@Query("select r from Creditrepayplan r join r.loan l join r.state state where l.id=?1 and state.stateCode='ACCOUNTED' and r.leftAmount > 0 and r.isOverdue = false")
	public Creditrepayplan findNormalPlan(Integer loanId);
}