...
public class LoanUtil {
     ...
     * @return
     */
    public Loan calculateFeebackup(BigDecimal principal,Set<Feebackup> feebackups, BigDecimal interest, Boolean interestAdvance) {
    	Loan loan = new Loan();
		...
		interestAdvance = interestAdvance == null ? false:interestAdvance;
		if (interestAdvance) {
			interest = interest == null ? new BigDecimal(0):interest;
			debtorRealityAmount = debtorRealityAmount.subtract(interest);//减去提前收的利息
		}
     ...
     * @return
     */
    public BigDecimal getDebtorInterest(BigDecimal principal,BigDecimal debtorInterest) {
		if (principal == null || debtorInterest == null) {
			return new BigDecimal(0);
		}
		return principal.multiply(debtorInterest).divide(BigDecimal.valueOf(12),2, BigDecimal.ROUND_UP);
	}
}
